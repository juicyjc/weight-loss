...
class AppTest(unittest.TestCase):
    def create_app(self):
        self.db_name = 'weightloss-api-test'
    ...
    def app_dict(self):
        return json.dumps(dict(
                app_id="weightloss_client",
                app_secret="weightloss_secret"
                ))
    ...
    def test_token_generation(self):
        ...
        missing_app_dict = json.dumps(dict(
                app_id="weightloss_client",
                app_secret="bad_weightloss_secret"
                ))
        rv = self.app.post('/apps/access_token/',
            data=missing_app_dict,
            content_type='application/json')
        ...
        rv = self.app.get('/weightloss/',
            headers={
                'X-APP-ID': 'weightloss_client',
                'X-APP-TOKEN': token},
            content_type='application/json')
        assert rv.status_code == 200
